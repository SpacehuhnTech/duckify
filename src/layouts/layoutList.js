import locale_us from './us.json'
import locale_de from './de.json'
import locale_ro from './ro.json'
import locale_it from './it.json'
import locale_es from './es.json'
import locale_si from './si.json'
import locale_sk from './sk.json'
import locale_be from './be.json'
import locale_fr from './fr.json'
import locale_cafr from './ca-fr.json'
import locale_pl from './pl.json'
import locale_pt from './pt.json'
import locale_cz from './cz.json'
import locale_dk from './dk.json'
import locale_gb from './gb.json'
import locale_ru from './ru.json'
import locale_in from './in.json'
import locale_nl from './nl.json'
import locale_chde from './ch-de.json'
import locale_chfr from './ch-fr.json'
import locale_no from './no.json'
import locale_se from './se.json'
import locale_is from './is.json'
import locale_fi from './fi.json'
import locale_lv from './lv.json'
import locale_ee from './ee.json'
import locale_lt from './lt.json'
import locale_ie from './ie.json'
import locale_gr from './gr.json'
import locale_hu from './hu.json'
import locale_bg from './bg.json'
import locale_ua from './ua.json'
import locale_ptbr from './pt-br.json'
import locale_esmx from './es-mx.json'

const layoutList = {
    'us': locale_us,
    'de': locale_de,
    'ro': locale_ro,
    'it': locale_it,
    'es': locale_es,
    'si': locale_si,
    'sk': locale_sk,
    'be': locale_be,
    'fr': locale_fr,
    'ca-fr': locale_cafr,
    'pl': locale_pl,
    'pt': locale_pt,
    'cz': locale_cz,
    'dk': locale_dk,
    'gb': locale_gb,
    'ru': locale_ru,
    'in': locale_in,
    'nl': locale_nl,
    'ch-de': locale_chde,
    'ch-fr': locale_chfr,
    'no': locale_no,
    'se': locale_se,
    'is': locale_is,
    'fi': locale_fi,
    'lv': locale_lv,
    'ee': locale_ee,
    'lt': locale_lt,
    'ie': locale_ie,
    'gr': locale_gr,
    'hu': locale_hu,
    'bg': locale_bg,
    'ua': locale_ua,
    'pt-br': locale_ptbr,
    'es-mx': locale_esmx,
}

export default layoutList